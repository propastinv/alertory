...
| Variable | Default | Description |
|---|---|---|
| `DISABLE_AUTH` | `false` | If `true`, skips OIDC entirely and serves every `/ui/` route unauthenticated (no `OIDC_*`/`APP_URL` needed). **Local development only - never set this on anything reachable by anyone else.** |

With `DISABLE_AUTH=true`, `OIDC_ISSUER_URL`/`OIDC_CLIENT_ID`/`OIDC_CLIENT_SECRET`/`APP_URL` are all ignored and the usual 503 "web UI is disabled" fallback doesn't apply either.
